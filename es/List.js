      ...
      return list.map(function (item, index) {
        ...
        return React.cloneElement(node, {
          ...
          ref: function ref(ele) {
            var divWidth = _this.listRef.current && _this.listRef.current.getBoundingClientRect().width;

            var rectwidth = ele && ele.getBoundingClientRect().width || 0;

            if (divWidth > _this.maxWidth) {
              _this.maxWidth = divWidth;
            }

            if (rectwidth > _this.maxWidth) {
              _this.maxWidth = rectwidth;
            }
